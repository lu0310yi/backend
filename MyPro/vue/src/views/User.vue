...
export default {
  ...
  methods: {
    load(){
      // fetch("http://localhost:8081/user/list/find?pagesize="+this.pageSize+"&pagenum="+this.pageNum)
      //     .then(res=> res.json())
      //     .then(res=>{console.log(res)
      //       this.tableData=res.data.list
      //       this.total=res.data.total
      //     })
      request.get("http://localhost:8081/user/list/find",{
        params: {
    ...
    saveUser(){
      request.post("http://localhost:8081/user/save"+this.form).then(
          res=>{
            if(res){
